...
const InterviewScreen = ({ sessionId, wsUrl, onFinish }) => {
    ...
    try {
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          ...
          // If we want real-time streaming to backend
  ...
  const handleFinish = async () => {
    try {
      await api.generateBiography(sessionId);
      onFinish();
    } catch (error) {
      console.error("Error generating biography:", error);
      onFinish(); // Proceed anyway for demo
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-between p-8 bg-memento-cream">
        ...
        <div className="h-32 flex items-center justify-center space-x-1 w-full">
          {Array.from({ length: 20 }).map((_, i) => (
            <motion.div
              key={i}
              animate={{
                height: isRecording ? [20, Math.random() * 80 + 20, 20] : 20
              }}
              transition={{
                repeat: Infinity,
                duration: 0.5,
        ...
        <div className="text-center space-y-6">
          <AnimatePresence mode='wait'>
            <motion.p
              ...
              exit={{ opacity: 0, y: -10 }}
              className="text-2xl font-medium text-memento-navy leading-relaxed italic"
            >
              "{aiQuestion}"
            </motion.p>
          </AnimatePresence>
      ...
      <footer className="w-full max-w-lg flex flex-col items-center space-y-6">
        ...
        >
          <Mic className={isRecording ? 'animate-bounce' : ''} size={32} />
